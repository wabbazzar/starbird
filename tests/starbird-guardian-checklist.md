...
- [ ] Evidence coverage: for every brand, each harm quest tag should have at least one supporting keyword in the `why` text. Run the keyword audit (`VALUE_KEYWORDS` mapping in the audit script) and flag entries where a tag exists but the why text has no matching evidence. This catches the "tag added without evidence" bug that occurs when the runner appends a harm tag to an existing entry without updating the why field.
